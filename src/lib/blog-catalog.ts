/**
 * Canonical blog catalog for listings and feeds.
 *
 * Single source of truth for every post that appears in a listing surface:
 *   - src/components/ui/blog-section.tsx  (/blog card grid)
 *   - src/app/rss.xml/route.ts            (RSS 2.0 feed items)
 *   - src/lib/markdown-generator.ts       (/blog markdown + llms-full.txt)
 *
 * Dynamic posts are authored in src/app/blog/[slug]/page.tsx (`posts`).
 * Standalone posts own their own page.tsx; they are catalogued here so that
 * feeds and machine-readable endpoints can see them too.
 */
