/**
 * LiquidEther — Production-grade WebGL fluid simulation (Navier-Stokes)
 *
 * Ported to TypeScript from the LiquidEther reference component.
 * Uses a self-contained vanilla Three.js implementation (NOT React Three Fiber).
 *
 * Features:
 * - Real-time fluid dynamics (advection, external force, viscosity, divergence, poisson, pressure)
 * - BFECC (Back-and-Forth Error Compensation and Correction) for sharper details
 * - Mouse interaction with smooth takeover transition
 * - Auto-demo driver when idle (auto-resumes after autoResumeDelay ms)
 * - Custom color palettes via DataTexture
 * - IntersectionObserver pauses render when off-screen
 * - ResizeObserver handles container resize
 *
 * Default palette is MONOCHROME (liquid mercury / liquid silk) to match the
 * TRIONN design system. Override via the `colors` prop.
 */
...
const DEFAULT_MONOCHROME_PALETTE = [
  "#0a0a0a",
  "#3a3a3a",
  "#888888",
  "#d8d8d8",
...
export function LiquidEther({
  ...
  resolution = 0.5,
  isBounce = false,
  colors = DEFAULT_MONOCHROME_PALETTE,
  style = {},
  className = "",
  autoDemo = true,
  autoSpeed = 0.5,
  ...
  useEffect(() => {
    ...
    class ShaderPass {
      ...
      // Base update is a no-op fallback; subclasses override with typed args.
      // Using a rest-tuple signature so subclasses can specialize the args.
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(args: {
        ...
        const uniforms = (this.mouse.material as THREE.RawShaderMaterial)
          .uniforms;
        (uniforms.force.value as THREE.Vector2).set(
          (Mouse.diff.x / 2) * mouse_force,
          (Mouse.diff.y / 2) * mouse_force
        );
        (uniforms.center.value as THREE.Vector2).set(
          Mouse.coords.x,
          Mouse.coords.y
        );
        (uniforms.scale.value as THREE.Vector2).set(cursor_size, cursor_size);
        super.update();
      }
    }
