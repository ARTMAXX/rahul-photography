...
function Header1() {
  const navigationItems = [
    ...
    {
      // Named for its contents (Apple: "specific labels beat generic
      // ones" — the old umbrella "More" answered nothing).
      title: "Studio",
  ...
  return (
    <header className={`w-full z-40 fixed top-0 left-0 bg-transparent transition-transform duration-300 ${isCylinderVisible ? "-translate-y-full" : "translate-y-0"}`}>
      <div className="container relative mx-auto min-h-20 flex gap-4 flex-row lg:grid lg:grid-cols-3 items-center">
                      ...
                      <NavigationMenuContent className="!w-[600px] p-5 bg-black/95 backdrop-blur-md border border-white/10">
                        <div className="flex flex-col lg:grid grid-cols-2 gap-6">
                          <div className="flex flex-col h-full justify-between">
                            ...
                            >
                              <Button size="sm" className="mt-6 bg-[#e83b2c] hover:bg-[#c92f22] text-white">
                                Let&apos;s Talk
                              </Button>
                            </a>
                          </div>
        ...
        {/* Right side - CTA. One primary action (Apple: one accent,
            defended — the old "Book a call" ghost linked to the SAME
            WhatsApp URL as the pill, two CTAs one destination). */}
        <div className="flex justify-end w-full gap-3 items-center">
          <a
            href="https://wa.me/917078939475"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 rounded-full bg-[#e83b2c] px-4 py-2 text-sm font-medium text-white hover:bg-[#c92f22] transition-colors whitespace-nowrap"
          >
