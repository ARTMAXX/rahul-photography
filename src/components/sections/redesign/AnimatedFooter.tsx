...
export function AnimatedFooter({
  headingLines = ["Rahul Chanda"],
  leftImage = "/best shots/Product image/product-watch-luxury.webp",
  rightImage = "/best shots/Food photo/food-biriyani.webp",
  background,
  textColor,
  charColor,
  hoverColor,
  ...
  const cc = charColor ?? "#8c1c13";
  const hc = hoverColor ?? "#e83b2c";
  ...
  return (
    <footer
      ref={rootRef}
      className={cn(
        "relative min-h-[100svh] w-full overflow-hidden text-white",
        className
      )}
    ...
    >
      ...
      {/* Display headings */}
      <div className="absolute inset-x-0 bottom-0 flex items-end justify-center gap-4 p-8 z-10">
        {headingLines.map((word, wi) => (
          <h2
            key={`${word}-${wi}`}
            aria-label={word}
            className="overflow-hidden font-serif leading-none tracking-tight pb-[0.15em] -mb-[0.15em] text-white"
